...
var stateHandlers = {
    startModeIntentHandlers : Alexa.CreateStateHandler(constants.states.START_MODE, {
        ...
        'LaunchRequest' : function () {
            // Initialize Attributes
              audioData.getRecordings((myData) =>{
              this.attributes['recordings'] = myData;
              this.attributes['playOrder'] =  Array.apply(null, {length: myData.length}).map(Number.call, Number);
        ...
        'PlayAudio' : function () {
            if (!this.attributes['playOrder']) {
                ...
                this.attributes['playOrder'] = Array.apply(null, {length: this.attributes['recordings'].length}).map(Number.call, Number);
                this.attributes['index'] = 0;
                this.attributes['offsetInMilliseconds'] = 0;
                this.attributes['loop'] = true;
                this.attributes['shuffle'] = false;
    ...
    resumeDecisionModeIntentHandlers : Alexa.CreateStateHandler(constants.states.RESUME_DECISION_MODE, {
         ...
         */
        'LaunchRequest' : function () {
            var message = 'You were listening to ' + this.attributes['recordings'][this.attributes['playOrder'][this.attributes['index']]].title +
                ' Would you like to resume?';
        ...
        'AMAZON.HelpIntent' : function () {
            var message = 'You were listening to ' + this.attributes['recordings'][this.attributes['index']].title +
                ' Would you like to resume?';
            var reprompt = 'You can say yes to resume or no to play from the top.';
            this.response.speak(message).listen(reprompt);
            this.emit(':responseReady');
...
var controller = function () {
    return {
        play: function () {
            ...
            var podcast = this.attributes['recordings'][this.attributes['playOrder'][this.attributes['index']]];
            var offsetInMilliseconds = this.attributes['offsetInMilliseconds'];
            // Since play behavior is REPLACE_ALL, enqueuedToken attribute need to be set to null.
            this.attributes['enqueuedToken'] = null;

            if (canThrowCard.call(this)) {
                var cardTitle = 'Birthday Greetings from ' + podcast.title;
                var cardContent = 'Playing birthday greetings from act' + podcast.title;
                this.response.cardRenderer(cardTitle, cardContent, null);
            }

            // this.response.speak("Message from " + podcast.title);
        ...
        playNext: function () {
            ...
            if (index === this.attributes['recordings'].length) {
            ...
            this.attributes['offsetInMilliseconds'] = 0;
            this.attributes['playbackIndexChanged'] = true;
            // this.response.speak("Next message from " + podcast.title);
            controller.play.call(this);
        },
        playPrevious: function () {
            ...
            if (index === -1) {
                if (this.attributes['loop']) {
                    index = this.attributes['recordings'].length - 1;
                } else {
                    // Reached at the end. Thus reset state to start mode and stop playing.
                    this.handler.state = constants.states.START_MODE;
...
function shuffleOrder(callback) {
    ...
    var array = Array.apply(null, {length: this.attributes['recordings'].length}).map(Number.call, Number);
